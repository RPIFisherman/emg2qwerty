...
class TransformerCTCModule(pl.LightningModule):
    def __init__(
        ...
        lr_scheduler: DictConfig,
        decoder: DictConfig,
        input_dim: int = 64,  # default will be overridden by config
        d_model: int = 512,
        nhead: int = 8,
        num_layers: int = 6,
    ):
        ...
        self.model = TransformerEncoderCTC(
            input_dim=input_dim,
            d_model=d_model,
            nhead=nhead,
            num_layers=num_layers
        ...
        self.ctc_loss = nn.CTCLoss(blank=charset().null_class)
        self.decoder = instantiate(decoder)
        # ...existing metrics and others...
        # Add metrics (using similar settings as TDSConvCTCModule)
        from torchmetrics import MetricCollection
        from emg2qwerty.metrics import CharacterErrorRates
        metrics = MetricCollection([CharacterErrorRates()])
        self.metrics = nn.ModuleDict({
